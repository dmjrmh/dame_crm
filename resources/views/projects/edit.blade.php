<x-app-layout>
  <x-slot name="header">
    <h2 class="text-xl font-semibold leading-tight">Edit Project</h2>
  </x-slot>


  <div class="max-w-6xl mx-auto p-6">
    @if (session('error'))
      <div class="mb-4 rounded-lg bg-red-300 p-3 text-red-700">
        {{ session('error') }}
      </div>
    @endif
    <form action="{{ route('projects.update', $deal) }}" method="POST" class="space-y-6">
      @csrf
      @method('PUT')
      <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          @php
            $source = old('source_type', $deal->lead_id ? 'lead' : ($deal->customer_id ? 'customer' : ''));
          @endphp
          <select id="source_type" name="source_type" class="w-full rounded-md border px-3 py-2" required>
            <option value="">—</option>
            <option value="lead" @selected($source === 'lead')>Lead</option>
            <option value="customer" @selected($source === 'customer')>Customer</option>
          </select>
        </div>
        ...
        <div id="lead_group" style="display:none">
          ...
          <select id="lead_id" name="lead_id" class="w-full rounded-md border px-3 py-2">
            ...
            @foreach ($leads as $lead)
              <option value="{{ $lead->id }}" @selected(old('lead_id', $deal->lead_id) == $lead->id)>
                {{ $lead->name }} — Phone: {{ $lead->contact }}
              </option>
            @endforeach
          </select>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <input name="expected_close_date" type="date" class="rounded-md border px-3 py-2"
            value="{{ old('expected_close_date', optional($deal->expected_close_date)->format('Y-m-d') ?? date('Y-m-d')) }}">
        </div>

        <div class="col-span-full">
          <label for="notes" class="block text-sm/6 font-medium text-gray-900">Notes</label>
          <div class="mt-2">
            <textarea id="notes" name="notes" rows="3" class="block w-full rounded-md bg-white px-3 py-1.5">{{ old('notes', $deal->notes ?? $deal->lead?->notes) }}</textarea>
          </div>
          @error('notes')
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 items-start deal-row">
              <div class="md:col-span-4">
                ...
                <select name="items[{{ $idx }}][product_id]"
                  class="product w-full rounded-md border px-3 py-2" required>
                  <option value="">—</option>
              ...
              <div class="md:col-span-1">
                ...
                <input type="number" name="items[{{ $idx }}][quantity]" min="1"
                  value="{{ $row['quantity'] }}" class="qty text-right w-full rounded-md border px-3 py-2">
              </div>

              <div class="md:col-span-2">
                <label class="block text-sm mb-1">Unit Price</label>
                <input type="number" step="0.01" min="0" name="items[{{ $idx }}][unit_price]"
                  value="{{ $row['unit_price'] }}" class="unit-price w-full rounded-md border px-3 py-2 text-right">
              </div>
              ...
              <input type="hidden" name="items[{{ $idx }}][deal_id]" value="{{ $deal->id }}">
      ...
      <div class="px-4 py-3 border-t flex items-center justify-end">
        ...
        <input id="amount" name="amount" type="number" step="0.01" min="0"
          class="text-right rounded-md border px-3 py-2" value="{{ old('amount', (float) $deal->amount) }}" readonly>
      </div>
      @error('amount')
      ...
      if (addBtn && wrapper) {
        addBtn.addEventListener('click', () => {
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 items-start deal-row">
            ...
            <input type="hidden" name="items[${itemIndex}][deal_id]" value="{{ $deal->id }}">
          </div>`;
